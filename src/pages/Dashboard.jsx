...
import { 
  ...
  Home, ClipboardList, LogOut, PlusCircle 
} from 'lucide-react';
...
export default function Dashboard({ onNavigate }) {
  ...
  // 2. Logic to add item to Local Storage (Offline Mode)
      ...
      <div className="w-full max-w-[400px] h-[800px] bg-slate-50 relative overflow-hidden flex flex-col sm:rounded-[2.5rem] shadow-2xl border-slate-800 sm:border-8">
        ...
        <header className="bg-emerald-600 text-white px-6 pt-6 pb-8 rounded-b-[2rem] shadow-md z-10 relative">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold tracking-tight uppercase italic">Bantay Presyo</h1>
              <div className="flex items-center gap-1 mt-1 bg-emerald-700/50 rounded-lg px-2 py-1 w-max">
                ...
                >
                  ...
                  <option value="Metro Manila" className="text-slate-800">Manila</option>
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar pb-24 -mt-4 z-20">
          ...
          <div className="px-5 pt-2 mb-4 flex gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Maghanap ng bilihin..." className="w-full bg-white border border-slate-200 py-3 pl-11 pr-4 rounded-full shadow-sm focus:outline-none text-sm" />
            </div>
          ...
          <div className="px-5">
            <h2 className="text-slate-800 font-semibold mb-3 text-sm tracking-wide uppercase">Market Feed</h2>
            {loading ? (
              <div className="text-center py-20 text-slate-400">Loading data...</div>
            ) : (
              <div className="space-y-3">
                {items.map((item) => (
                  <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center group active:scale-95 transition-transform">
                    <div className="flex items-center gap-4">
                      <div className="h-12 w-12 bg-slate-50 text-slate-600 rounded-xl flex items-center justify-center text-xl border border-slate-100">
                        {item.icon}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <div className="text-slate-300 text-[10px]">/ kilo</div>
                      </div>
                      <button 
                        onClick={() => handleAddToList(item)}
                        className="text-emerald-500 bg-emerald-50 p-2 rounded-full hover:bg-emerald-100 transition-colors"
                      >
                        <PlusCircle size={24} />
                      </button>
                    </div>
        ...
        {/* Bottom Nav */}
        <nav className="absolute bottom-0 w-full bg-white border-t border-slate-100 px-6 py-4 flex justify-between items-center z-30">
          <div className="flex flex-col items-center text-emerald-600 gap-1.5 cursor-pointer">
            <Home size={22} />
            <span className="text-[10px] font-bold">Home</span>
          </div>
          <div onClick={() => onNavigate('list')} className="flex flex-col items-center text-slate-400 hover:text-emerald-500 gap-1.5 cursor-pointer">
            <ClipboardList size={22} />
            <span className="text-[10px] font-semibold">Listahan</span>
          </div>
          <div onClick={handleLogout} className="flex flex-col items-center text-slate-400 hover:text-rose-500 gap-1.5 cursor-pointer">
            <LogOut size={22} />
